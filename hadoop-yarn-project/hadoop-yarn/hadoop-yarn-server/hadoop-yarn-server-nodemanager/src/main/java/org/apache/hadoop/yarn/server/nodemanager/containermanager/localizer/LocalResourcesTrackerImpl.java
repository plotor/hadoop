...
class LocalResourcesTrackerImpl implements LocalResourcesTracker {
  ...
  public LocalResourcesTrackerImpl(String user,
                                   ApplicationId appId,
    ...
    this(user, appId, dispatcher, new ConcurrentHashMap<>(),
        useLocalCacheDirectoryManager, conf, stateStore, dirHandler);
  }

  LocalResourcesTrackerImpl(String user,
                            ApplicationId appId,
                            Dispatcher dispatcher,
    ...
    if (this.useLocalCacheDirectoryManager) {
      ...
      inProgressLocalResourcesMap = new ConcurrentHashMap<>();
    }
    this.conf = conf;
    ...
    case RELEASE:
      if (null == rsrc) {
        ...
        LOG.info("Container " + relEvent.getContainer()
            + " sent RELEASE event on a resource request " + req + " not present in cache.");
        return;
      }
      break;
    ...
    if (rsrc.getRefCount() > 0
      ...
      if (ResourceState.LOCALIZED.equals(rsrc.getState())) {
        FileDeletionTask deletionTask = new FileDeletionTask(
            delService, getUser(), getPathToDelete(rsrc.getLocalPath()), null);
        delService.delete(deletionTask);
      }
      removeResource(rem.getRequest());
      LOG.info("Removed {} from localized cache", rsrc.getLocalPath());
      return true;
    }
  }
