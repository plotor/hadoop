...
public class Job extends JobContextImpl implements JobContext, AutoCloseable {
        ...
        try {
            this.status = ugi.doAs(new PrivilegedExceptionAction<JobStatus>() {
                ...
                public JobStatus run() throws IOException, InterruptedException {
                    return cluster.getClient().getJobStatus(Job.this.getJobID());
                }
            });
    ...
    public TaskReport[] getTaskReports(TaskType type) throws IOException, InterruptedException {
        ...
        return ugi.doAs(new PrivilegedExceptionAction<TaskReport[]>() {
            ...
            public TaskReport[] run() throws IOException, InterruptedException {
                return cluster.getClient().getTaskReports(Job.this.getJobID(), tmpType);
            }
        });
    }
    ...
    public void killJob() throws IOException {
        this.ensureState(JobState.RUNNING);
        try {
            cluster.getClient().killJob(this.getJobID());
        } catch (InterruptedException ie) {
            throw new IOException(ie);
        }
    }
    ...
    public void setPriority(JobPriority jobPriority) throws IOException, InterruptedException {
        if (state == JobState.DEFINE) {
            if (jobPriority == JobPriority.UNDEFINED_PRIORITY) {
                conf.setJobPriorityAsInteger(this.convertPriorityToInteger(jobPriority));
            } else {
                conf.setJobPriority(org.apache.hadoop.mapred.JobPriority.valueOf(jobPriority.name()));
            }
        } else {
            this.ensureState(JobState.RUNNING);
            final int tmpPriority = this.convertPriorityToInteger(jobPriority);
            ugi.doAs(new PrivilegedExceptionAction<Object>() {
                @Override
                public Object run() throws IOException, InterruptedException {
                    cluster.getClient()
                            .setJobPriority(Job.this.getJobID(), Integer.toString(tmpPriority));
                    return null;
                }
            });
        }
    ...
    public TaskCompletionEvent[] getTaskCompletionEvents(final int startFrom,
        ...
        return ugi.doAs(new PrivilegedExceptionAction<TaskCompletionEvent[]>() {
            ...
            public TaskCompletionEvent[] run() throws IOException, InterruptedException {
                return cluster.getClient().getTaskCompletionEvents(Job.this.getJobID(),
                        startFrom, numEvents);
            }
        });
    }
    ...
    getTaskCompletionEvents(final int startFrom) throws IOException {
        try {
            TaskCompletionEvent[] events = this.getTaskCompletionEvents(startFrom, 10);
        ...
        try {
            return ugi.doAs(new PrivilegedExceptionAction<Counters>() {
                ...
                public Counters run() throws IOException, InterruptedException {
                    return cluster.getClient().getJobCounters(Job.this.getJobID());
                }
            });
        } catch (InterruptedException ie) {
            throw new IOException(ie);
    ...
    private void setUseNewAPI() throws IOException {
        ...
        if (conf.getUseNewMapper()) {
            ...
            this.ensureNotSet(INPUT_FORMAT_CLASS_ATTR, mode);
            this.ensureNotSet(MAP_CLASS_ATTR, mode);
            if (numReduces != 0) {
                this.ensureNotSet(PARTITIONER_CLASS_ATTR, mode);
            } else {
                this.ensureNotSet(OUTPUT_FORMAT_CLASS_ATTR, mode);
            }
        }
        if (numReduces != 0) {
            ...
            if (conf.getUseNewReducer()) {
                ...
                this.ensureNotSet(OUTPUT_FORMAT_CLASS_ATTR, mode);
                this.ensureNotSet(REDUCE_CLASS_ATTR, mode);
            }
        }
    }
     ...
     * archives.
     */
    private static void setSharedCacheUploadPolicies(Configuration conf, Map<String, Boolean> policies, boolean areFiles) {
        if (policies != null) {
    ...
    public void submit()
        ...
        final JobSubmitter submitter =
                this.getJobSubmitter(cluster.getFileSystem(), cluster.getClient());
    ...
    public boolean monitorAndPrintJob()
        ...
        IntegerRanges mapRanges = this.getProfileTaskRange(true);
        IntegerRanges reduceRanges = this.getProfileTaskRange(false);
        ...
        while (!this.isComplete() || !reportedAfterCompletion) {
            ...
            if (!reportedUberMode) {
                ...
                LOG.info("Job " + jobId + " running in uber mode : " + this.isUber());
            }
            String report =
                    (" map " + StringUtils.formatPercent(this.mapProgress(), 0) +
                            " reduce " +
                            StringUtils.formatPercent(this.reduceProgress(), 0));
            ...
            TaskCompletionEvent[] events =
                    this.getTaskCompletionEvents(eventCounter, 10);
